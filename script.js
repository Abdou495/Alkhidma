...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const slider = document.getElementById('categoryProducts');

    if (slider) {
        // Fonction pour initier le mouvement
        slider.addEventListener('touchstart', (e) => {
            startX = e.touches[0].pageX; // Position du doigt au début
            scrollLeft = slider.scrollLeft; // Position actuelle du défilement
        });

        // Fonction pour gérer le mouvement du doigt
        slider.addEventListener('touchmove', (e) => {
            const distance = e.touches[0].pageX - startX; // Distance parcourue par le doigt
            slider.scrollLeft = scrollLeft - distance; // Décale l'élément en fonction du mouvement
        });

        // Empêcher le défilement vertical pour le swipe horizontal
